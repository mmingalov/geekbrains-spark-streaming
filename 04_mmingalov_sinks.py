...
def memory_sink(df, freq):
    return df.writeStream.format("memory") \
        .queryName("mmingalov_memory_sink_table") \
        .trigger(processingTime='%s seconds' % freq ) \
        .start()

stream = memory_sink(parsed_orders,5)
spark.sql("select * from mmingalov_memory_sink_table").show()
spark.sql("""select *, current_timestamp() as my_extra_column from mmingalov_memory_sink_table order_status = "delivered" """).show()
...
def file_sink(df, freq):
    return df.writeStream.format("parquet") \
        ...
        .option("checkpointLocation", "mmingalov_parquet_checkpoint") \
        .start()
...
#правильно чистить через темповую директорию
def compact_directory(path):
    df_to_compact = spark.read.parquet(path + "/*.parquet")
    df_to_compact.persist() #для сохранения в памяти
    df_to_compact.count()  #для активации персиста, поскольку он lazy и не выполнится сам собой
...
stream = kafka_sink(extended_orders,5)
stream.stop()


def kafka_sink_json(df, freq):
    #в Kafka пишем KEY и VALUE. Чтбы получить VALUE, воспользуемся функцией STRUCT()
